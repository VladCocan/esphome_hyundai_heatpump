...
constexpr uint32_t entity_fields(uint32_t dc_idx = 0, uint32_t uom_idx = 0, uint32_t icon_idx = 0,
             EntityCategory category = ENTITY_CATEGORY_NONE) {
  return (dc_idx << ENTITY_FIELD_DC_SHIFT) | (uom_idx << ENTITY_FIELD_UOM_SHIFT) |
    (icon_idx << ENTITY_FIELD_ICON_SHIFT) |
    (static_cast<uint32_t>(category) << ENTITY_FIELD_ENTITY_CATEGORY_SHIFT);
}
...
sensor::Sensor *make_sensor(const char *name, uint32_t entity_fields = 0, int8_t accuracy_decimals = -1,
                            sensor::StateClass state_class = sensor::STATE_CLASS_NONE) {
  auto *entity = new NamedSensor();
  entity->init(name, entity_fields);
  if (accuracy_decimals >= 0) {
    entity->set_accuracy_decimals(accuracy_decimals);
  }
  if (state_class != sensor::STATE_CLASS_NONE) {
    entity->set_state_class(state_class);
  }
  App.register_sensor(entity);
  return entity;
}

binary_sensor::BinarySensor *make_binary_sensor(const char *name, uint32_t entity_fields = 0) {
...
text_sensor::TextSensor *make_text_sensor(const char *name, uint32_t entity_fields = 0) {
...
void HyundaiHeatPumpDevice::setup() {
  auto make_bit_switch = [this](HyundaiBitSwitch *&target, const char *name, uint16_t reg, uint16_t *cache,
                                uint16_t bitmask, uint32_t extra_entity_fields = 0) {
    auto *entity = new NamedHyundaiBitSwitch();
    entity->init(name, extra_entity_fields |
                           (uint32_t(ENTITY_CATEGORY_CONFIG) << ENTITY_FIELD_ENTITY_CATEGORY_SHIFT));
  ...
  auto make_number = [this](HyundaiNumber *&target, const char *name, float min_value, float max_value, float step,
                            const HyundaiNumber::WriteFn &write_fn = nullptr, uint32_t extra_entity_fields = 0) {
  ...
  auto make_select = [this](HyundaiSelect *&target, const char *name, const std::initializer_list<const char *> &options,
                            const HyundaiSelect::WriteFn &write_fn = nullptr, uint32_t extra_entity_fields = 0) {
  ...
  this->s_comp_freq_ = make_sensor("Compressor Operating Frequency",
                                   entity_fields(HYUNDAI_DC_FREQUENCY, HYUNDAI_UOM_HZ), 0,
                                   sensor::STATE_CLASS_MEASUREMENT);
  this->s_fan_speed_ = make_sensor("Fan Speed", entity_fields(0, HYUNDAI_UOM_RPM), 0,
                                   sensor::STATE_CLASS_MEASUREMENT);
  this->s_water_inlet_temp_ = make_sensor("Water Inlet Temperature",
                                          entity_fields(HYUNDAI_DC_TEMPERATURE, HYUNDAI_UOM_C), 0,
                                          sensor::STATE_CLASS_MEASUREMENT);
  ...
  this->s_water_tank_t5_ = make_sensor("Water Tank Temperature T5",
                                       entity_fields(HYUNDAI_DC_TEMPERATURE, HYUNDAI_UOM_C), 0,
                                       sensor::STATE_CLASS_MEASUREMENT);
  this->s_current_fault_ = make_sensor("Current Fault", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->s_fault_1_ = make_sensor("Fault 1", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->s_fault_2_ = make_sensor("Fault 2", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->s_fault_3_ = make_sensor("Fault 3", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->s_water_flow_ = make_sensor("Water Flow", 0, 2, sensor::STATE_CLASS_MEASUREMENT);
  this->s_elec_consumption_ = make_sensor("Electricity Consumption",
                                          entity_fields(HYUNDAI_DC_POWER, HYUNDAI_UOM_KW), 2,
                                          sensor::STATE_CLASS_MEASUREMENT);
  this->s_power_output_ = make_sensor("Power Output", entity_fields(HYUNDAI_DC_POWER, HYUNDAI_UOM_KW), 2,
                                      sensor::STATE_CLASS_MEASUREMENT);
  this->s_forced_ibh1_ = make_sensor("Forced Hydraulic Module Rear Electric Heater 1");
  this->s_t_sg_max_ = make_sensor("t_SG_MAX", entity_fields(HYUNDAI_DC_TEMPERATURE, HYUNDAI_UOM_C), 0,
                                  sensor::STATE_CLASS_MEASUREMENT);
  this->s_cop_ = make_sensor("Coefficient of Performance", 0, 2, sensor::STATE_CLASS_MEASUREMENT);
  this->s_water_temp_delta_ = make_sensor("Water Temperature Delta",
                                          entity_fields(HYUNDAI_DC_TEMPERATURE_DELTA, HYUNDAI_UOM_C), 1,
                                          sensor::STATE_CLASS_MEASUREMENT);
  this->s_t1s_dhw_ = make_sensor("T1S DHW", entity_fields(HYUNDAI_DC_TEMPERATURE, HYUNDAI_UOM_C), 1,
                                 sensor::STATE_CLASS_MEASUREMENT);
  this->s_comp_starts_per_hour_ = make_sensor("Compressor Starts Per Hour", entity_fields(0, HYUNDAI_UOM_PER_H), 0,
                                              sensor::STATE_CLASS_MEASUREMENT);
  this->s_active_state_map_ = make_sensor("Active State Map", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));

  this->bs_defrosting_ = make_binary_sensor("Status BIT 1 Defrosting", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->bs_oil_return_ = make_binary_sensor("Status BIT 1 Oil Return", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->bs_pump_o_ = make_binary_sensor("Load Output External Circulation Pump PUMP_O",
                                        entity_fields(0, 0, HYUNDAI_ICON_PUMP));
  this->bs_sv1_ = make_binary_sensor("Load Output SV 1", entity_fields(0, 0, HYUNDAI_ICON_VALVE));
  this->bs_run_ = make_binary_sensor("Load Output RUN", entity_fields(0, 0, HYUNDAI_ICON_HVAC));
  this->bs_compressor_running_ = make_binary_sensor("Compressor Running", entity_fields(0, 0, HYUNDAI_ICON_HVAC));
  this->bs_hp_running_ = make_binary_sensor("Heat pump running", entity_fields(0, 0, HYUNDAI_ICON_HVAC));
  this->bs_tbh_enabled_ = make_binary_sensor("Status TBH Enabled", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->bs_ahs_enabled_ = make_binary_sensor("Status AHS Enabled", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->bs_ibh_enabled_ = make_binary_sensor("Status IBH Enabled", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->bs_dhw_operation_ = make_binary_sensor("Status DHW Operation",
                                               entity_fields(0, 0, HYUNDAI_ICON_WATER_THERMOMETER));
  this->bs_heat_operation_ = make_binary_sensor("Status Heating Operation",
                                                entity_fields(0, 0, HYUNDAI_ICON_RADIATOR));
  this->bs_cool_operation_ = make_binary_sensor("Status Cooling Operation",
                                                entity_fields(0, 0, HYUNDAI_ICON_SNOWFLAKE));

  make_bit_switch(this->sw_room_temp_ctrl_, "ON/OFF Room Temperature Control", REG_SWITCHES_0, &this->cache_reg_0x00,
                  BIT_ROOM_TEMP_CTRL, entity_fields(0, 0, HYUNDAI_ICON_THERMOMETER));
  make_bit_switch(this->sw_flow_ctrl_zone1_, "ON/OFF Water Flow Temperature Control Zone 1", REG_SWITCHES_0,
                  &this->cache_reg_0x00, BIT_FLOW_CTRL_ZONE1, entity_fields(0, 0, HYUNDAI_ICON_WATER_THERMOMETER));
  make_bit_switch(this->sw_power_dhw_t5s_, "ON/OFF Power DHW T5S", REG_SWITCHES_0, &this->cache_reg_0x00,
                  BIT_POWER_DHW_T5S, entity_fields(0, 0, HYUNDAI_ICON_WATER_THERMOMETER));
  make_bit_switch(this->sw_flow_ctrl_zone2_, "ON/OFF Water Flow Temperature Control Zone 2", REG_SWITCHES_0,
                  &this->cache_reg_0x00, BIT_FLOW_CTRL_ZONE2, entity_fields(0, 0, HYUNDAI_ICON_WATER_THERMOMETER));
  make_bit_switch(this->sw_disinfect_, "Function Setting Disinfect", REG_FUNC_SETTINGS_5, &this->cache_reg_0x05,
                  BIT_DISINFECT, entity_fields(0, 0, HYUNDAI_ICON_SHIELD_CHECK));
  make_bit_switch(this->sw_silent_mode_, "Function Setting Silent Mode", REG_FUNC_SETTINGS_5, &this->cache_reg_0x05,
                  BIT_SILENT_MODE, entity_fields(0, 0, HYUNDAI_ICON_VOLUME_OFF));
  make_bit_switch(this->sw_eco_mode_, "Function Setting ECO Mode", REG_FUNC_SETTINGS_5, &this->cache_reg_0x05,
                  BIT_ECO_MODE, entity_fields(0, 0, HYUNDAI_ICON_LEAF));
  make_bit_switch(this->sw_enswitchpdc_, "EnSwitchPDC", REG_ENSWITCHPDC, &this->cache_reg_0x112, 0x0001,
                  entity_fields(0, 0, HYUNDAI_ICON_POWER_PLUG));
  ...
  make_number(this->n_t1s_zone1_, "Set Water Temperature T1S Zone 1", 5.0f, 60.0f, 1.0f,
              ...
              entity_fields(HYUNDAI_DC_TEMPERATURE, HYUNDAI_UOM_C, HYUNDAI_ICON_WATER_THERMOMETER));
  ...
  make_number(this->n_air_temp_ts_, "Air Temperature Ts", 17.0f, 30.0f, 0.5f,
              ...
              entity_fields(HYUNDAI_DC_TEMPERATURE, HYUNDAI_UOM_C, HYUNDAI_ICON_THERMOMETER));
  ...
  make_number(this->n_wc_curve_zone1_, "Weather Compensation Curve Zone 1", 1.0f, 9.0f, 1.0f,
              ...
              entity_fields(0, 0, HYUNDAI_ICON_TUNE));
  ...
  make_number(this->n_dt1s5_, "dT1S5", 5.0f, 40.0f, 1.0f,
              ...
              entity_fields(HYUNDAI_DC_TEMPERATURE_DELTA, HYUNDAI_UOM_C, HYUNDAI_ICON_THERMOMETER_LINES));
  make_number(this->n_t_t4_fresh_h_, "t_T4 FRESH_H", 0.5f, 6.0f, 0.5f,
              ...
              }, entity_fields(HYUNDAI_DC_DURATION, HYUNDAI_UOM_H, HYUNDAI_ICON_TIMER));
  ...
  make_select(this->sel_operational_mode_, "Operational Mode", {"Heat", "Cool", "Auto"},
              ...
              }, entity_fields(0, 0, HYUNDAI_ICON_HVAC));
  ...
  make_select(this->sel_room_temp_ctrl_, "Room Temperature Control Select", {"Off", "On"},
              ...
              entity_fields(0, 0, HYUNDAI_ICON_THERMOMETER));
  make_select(this->sel_power_input_limit_, "Power Input Limitation Type", {"None", "1", "2", "3", "4", "5", "6", "7", "8"},
              ...
              entity_fields(0, 0, HYUNDAI_ICON_GAUGE));
  ...
  make_select(this->sel_solar_function_, "Solar Function Mode", {"No Function", "Solar + Heat Pump", "Only Solar"},
              ...
              entity_fields(0, 0, HYUNDAI_ICON_SOLAR));
  ...
  this->ts_current_fault_code_ = make_text_sensor("Current Fault Error Code",
                                                  entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->ts_current_fault_desc_ = make_text_sensor("Current Fault Error Code Description",
                                                  entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->ts_fault_1_code_ = make_text_sensor("Fault 1 Error Code", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->ts_fault_2_code_ = make_text_sensor("Fault 2 Error Code", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->ts_fault_3_code_ = make_text_sensor("Fault 3 Error Code", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->ts_machine_type_ = make_text_sensor("MachineType", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->ts_hm_submodel_ = make_text_sensor("Hydraulic Module Sub-Model", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
  this->ts_product_code_ = make_text_sensor("Home Appliance Product Code", entity_fields(0, 0, ENTITY_CATEGORY_DIAGNOSTIC));
